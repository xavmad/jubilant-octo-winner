...
function activateGroup(project) {
  ...
  setTimeout(() => {
    ...
    }, 420);  // Delay image formation for 1 second
    ...
    setTimeout(() => {
      // Reveal the description only after the zoom-in transition is complete
      ...
      // Unlock the camera only after the transition is finished and description is visible
      cameraLocked = false;

    }, zoomDuration); // Ensure that the description is revealed after the zoom-in transition

  }, 1000); // Initial delay for the zoom-out and image formation
}
