...
function activateGroup(project) {
  ...
  setTimeout(() => {
    ...
    // Add a delay before images start to form (e.g., 1 second)
    setTimeout(() => {
      groupImages.forEach((img, i) => {
        ...
        // Slow down image formation by increasing transition duration
        img.style.transition = "transform 1s ease"; // 1s transition for smooth formation
        img.style.transform =
          `translate(${x}px, ${y}px) translate(-50%, -50%)`;
      });
    }, 1000);  // Delay the start of the image formation by 1 second
    ...
    setTimeout(() => {
      ...
      cameraLocked = false;  // Ensure camera is unlocked after transition

    }, zoomDuration); // This should match the transition duration

  }, 1000); // Adjust the timing of the initial transition
