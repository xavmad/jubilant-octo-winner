...
function activateGroup(project) {
  ...
  if (cameraLocked || isTransitioning) return;  // prevent starting new transition if one is in progress
  cameraLocked = true;
  isTransitioning = true;  // Set the flag to prevent new transitions
  ...
  setTimeout(() => {
    ...
    function cameraLoop() {
      if (progress < zoomDuration) {
        ...
        console.log("Progress:", progress);  // Debugging progress
      }

      applyTransform();
      if (progress < zoomDuration) {
        requestAnimationFrame(cameraLoop);  // Continue the loop until transition completes
      }
    }
    ...
    setTimeout(() => {
      cameraLocked = false;
      isTransitioning = false;  // Reset the flag after the transition finishes
    }, zoomDuration);
