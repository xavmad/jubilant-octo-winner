...
function activateGroup(project) {
  ...
  // Immediately hide the description box when the group is activated (before zoom)
  ...
  setTimeout(() => {
    ...
    // Now, set targetScale to 1 for the zoom-in transition to its original size
    targetScale = 1;

    // Start the camera animation with the adjusted target values
    groupCameraLoop(); // Start the camera animation
    ...
    setTimeout(() => {
      ...
      // Now show the description after the transition ends
      descriptions.classList.add("visible"); // Show the description after zoom-in is complete
    }, zoomDuration);
