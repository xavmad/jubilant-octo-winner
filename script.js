...
function groupCameraLoop() {
  ...
  if (progress < zoomDuration) {
    ...
    // Use linear interpolation instead of easing for direct zoom
    const t = progress / zoomDuration;

    // Apply the linear interpolation directly (no easing, straight line)
    originX = targetOriginX * t + originX * (1 - t);
...
function activateGroup(project) {
  ...
  // Save the initial target origin for later zoom-in reference
  if (initialTargetOriginX === null && initialTargetOriginY === null) {
    initialTargetOriginX = targetOriginX;
    initialTargetOriginY = targetOriginY;
  }
  ...
  setTimeout(() => {
    ...
    const CENTER_X = -window.innerWidth * 0.25;  // Adjusted this value to center-left the group
    const CENTER_Y = 0;
    ...
    const screenAnchorX = window.innerWidth * 0.5;  // Focus camera on the center of the screen
    const screenAnchorY = window.innerHeight * 0.5;

    targetScale = 1;
    targetOriginX = initialTargetOriginX;  // Use the same origin X from zoom-out
    targetOriginY = initialTargetOriginY;  // Use the same origin Y from zoom-out
