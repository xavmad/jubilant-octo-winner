...
setTimeout(() => {
  ...
  const IMAGE_WIDTH = 320; // Ensure the image width is set to 320px
  const MIN_GAP = 40;
  ...
  const SPACING = (IMAGE_WIDTH + MIN_GAP) / density;
  ...
  setTimeout(() => {
    groupImages.forEach((img, i) => {
      ...
      // Maintain the aspect ratio by adjusting height based on the natural image ratio
      const aspectRatio = img.naturalWidth / img.naturalHeight;
      img.style.height = (IMAGE_WIDTH / aspectRatio) + "px"; // Calculate height based on the aspect ratio
