...
function activateGroup(project) {
  ...
  // Delay the image formation slightly and start the transitions
  setTimeout(() => {
    ...
    // **Camera Transition**: To frame the group on the screen
    const screenAnchorX = window.innerWidth * 0.28; // Adjust for screen positioning
    const screenAnchorY = window.innerHeight * 0.5; // Center vertically
    ...
    // **After the camera animation and image formation, show the description**
    setTimeout(() => {
      console.log("Showing project description now...");
      descriptions.classList.add("visible");

      // Show the project description based on the selected project
      Array.from(descriptions.children).forEach(desc => {
        console.log("Checking description visibility: ", desc.dataset.project === project);
        desc.style.display = desc.dataset.project === project ? "block" : "none";
      });

      // Unlock the camera for further interaction
      cameraLocked = false;
    }, zoomDuration + 400);  // Ensure description shows after zoom and image transition
