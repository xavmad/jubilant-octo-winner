...
function activateGroup(project) {
  ...
  if (cameraLocked || isTransitioning) {
  ...
  cameraLocked = true; // Lock the camera
...
let progress = 0; // Current progress of transition
let isTransitioning = true; // Flag to track if a transition is in progress
let zoomDuration = 1500; // Set the desired zoom duration (1.5 seconds)

function cameraLoop() {
  // Only run the loop if transitioning
...
}, zoomDuration); // Ensure this timeout matches the zoom duration
