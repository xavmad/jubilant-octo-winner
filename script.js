...
function activateGroup(project) {
  ...
  // Set timeouts for zoom out, image formation, and camera transition
  setTimeout(() => {
    ...
    const CENTER_X = -window.innerWidth * 0.4;  // Positions at center-left
    const CENTER_Y = 0;
	...
	let lastTime = Date.now(); // Initialize lastTime variable
    ...
    function cameraLoop() {
      ...
      // Debug log to track progress and camera state
      console.log('Progress:', progress, 'Easing Factor:', easingFactor);
      console.log('Camera State:', { originX, originY, scale });
    }
