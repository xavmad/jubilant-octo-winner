...
let progress = 0; // Track the current progress of the transition
...
function cameraLoop() {
  if (!isTransitioning) return; // Prevent running the loop if transition is not active
  ...
  if (progress < zoomDuration) {
    ...
    requestAnimationFrame(cameraLoop); // Keep running the loop
  } else {
...
function activateGroup(project) {
  ...
  isTransitioning = true;  // Set the flag that the transition is in progress
