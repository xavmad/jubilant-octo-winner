...
/* =====================================================
   DESCRIPTION POSITION RESET (SAVE DEFAULT)
===================================================== */

const descriptionBox = document.getElementById("project-descriptions");

// Save original position once
const originalDescPos = {
  top: descriptionBox.offsetTop,
  left: descriptionBox.offsetLeft
};

function resetDescriptionPosition() {
  if (!descriptionBox) return;

  // Restore centered layout
  descriptionBox.style.top = "50%";
  descriptionBox.style.left = "65vw";
...
const logoLinks = document.getElementById("logo-links");

function hideLogoLinks() {
  if (logoLinks) logoLinks.classList.add("hidden");
}

function showLogoLinks() {
  if (logoLinks) logoLinks.classList.remove("hidden");
}
...
function finishLoader() {
  ...
  // Let browser finish decoding + GPU upload
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      // Start animations only when ready
      cameraLoop();
      floatImages();
...
preloadImages.forEach(img => {
  ...
  function handleLoad() {
    ...
    // Force decode before counting as ready
    if (img.decode) {
      img.decode()
        .catch(() => {}) // ignore failures
        .finally(() => incrementLoader());
    } else {
      incrementLoader();
    }
  }
...
const FLOAT_SMOOTH  = IS_MOBILE ? 0 : 0.02;
...
let groupTransitionId = 0;
let currentGroupTransition = 0;
...
const x = IS_MOBILE ? Math.round(targetOriginX) : targetOriginX;
const y = IS_MOBILE ? Math.round(targetOriginY) : targetOriginY;

canvas.style.transform =
  `translate(${x}px, ${y}px) scale(${targetScale})`;
...
initializeCamera();
// cameraLoop will start after loader
...
images.forEach(img => {
  ...
  // Random velocity for floating motion in both directions
	img._vx = (Math.random() - 0.5) * FLOAT_SPEED;
	img._vy = (Math.random() - 0.5) * FLOAT_SPEED;
...
function activateGroup(project) {
	...
	groupTransitionId++;
	currentGroupTransition = groupTransitionId;
  ...
  storedOriginY = targetOriginY;
	  ...
	  // Save current floating positions before grouping
	preGroupPositions = images.map(img => ({
	  img,
	  x: img._x,
	  y: img._y,
	  tx: img._tx,
  ...
  function zoomOutAnim(now) {
	...
	if (currentGroupTransition !== groupTransitionId) return;
...
const groupImages = images
  .filter(i => i.dataset.project === project)
  .sort((a, b) => {
  ...
  const IMAGE_SIZE = Math.min(320, window.innerWidth * 0.5);
  const MIN_GAP = 40;
  ...
  groupImages.forEach((img, i) => {
    ...
    img.style.zIndex = i === 0 ? 4000 : 1000 + i;  // Ensure images are stacked correctly
    img.style.transform = `translate(${x}px, ${y}px) translate(-50%, -50%)`;
  });
...
descriptions.addEventListener("pointerdown", e => {
  ...
  // Lock current position in pixels (important)
  descriptions.style.left = rect.left + "px";
  descriptions.style.top  = rect.top  + "px";

  // Remove any centering transform
  descriptions.style.transform = "none";

  // Store where inside the box we clicked
...
window.addEventListener("pointermove", e => {
  ...
  descriptions.style.left = x + "px";
  descriptions.style.top  = y + "px";
});
...
window.addEventListener("keydown", (e) => {
  if (e.key === "Escape") {
	...
	// Restore pre-group floating positions
	if (preGroupPositions) {
	  preGroupPositions.forEach(p => {
...
function handleResize() {
  ...
  // Update constants
  CAMERA_CENTER_X = originX;
  CAMERA_CENTER_Y = originY;

  // Recalculate world size
  worldScaleX = w * 1.6;
  worldScaleY = h * 1.6;

  boundaryScaleX = worldScaleX * 1.2;
  boundaryScaleY = worldScaleY * 1.2;

  // Reposition floating images
  images.forEach(img => {
    img._x = (Math.random() - 0.5) * w;
    img._y = (Math.random() - 0.5) * h;
