...
function activateGroup(project) {
  ...
  // Show project description immediately
  ...
  // Log to check if it's reaching this point
  console.log("Description should now be visible");

  // Simple image formation without any transition effects
  const groupImages = images.filter(i => i.dataset.project === project);

  groupImages.forEach((img, i) => {
    img.style.transition = "none";  // Disable transition for debugging
    img.style.transform = `translate(-50%, -50%)`;  // Keep images at the center
    img.style.opacity = 1;  // Ensure full opacity
  });

  // Log to check image visibility and formation
  console.log("Images should be visible and in their final positions.");

  // Unlock the camera after this step (for debugging, no complex transition)
  cameraLocked = false;
}


    /* ==========================================
       CAMERA FRAMES GROUP — STABLE SCREEN ANCHOR
    ========================================== */
    ...
    // **Immediately reveal description after zoom-in transition**
    setTimeout(() => {
      descriptions.classList.add("visible");
      ...
      // Unlock the camera after the description becomes visible
      cameraLocked = false;

    }, zoomDuration); // Ensure description visibility matches zoom duration
