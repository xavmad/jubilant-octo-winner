...
let transitionInProgress = false; // Track whether a transition is in progress
...
function activateGroup(project) {
  ...
  if (cameraLocked || transitionInProgress) return; // Prevent re-entry during transition
  cameraLocked = true;
  transitionInProgress = true;  // Mark transition as in progress
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      transitionInProgress = false; // Mark transition as complete
      cameraLocked = false;
      progress = 0; // Reset progress for the next transition
    }, zoomDuration);
