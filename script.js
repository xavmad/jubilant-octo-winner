...
// In the `setTimeout` after the zoom-out, make sure the images are set to 320px width and height
setTimeout(() => {
  const groupImages = images.filter(i => i.dataset.project === project);
  const COUNT = groupImages.length;
  const IMAGE_SIZE = 320; // Ensure image size is set to 320px
  const MIN_GAP = 40;
  ...
  setTimeout(() => {
    groupImages.forEach((img, i) => {
      // Ensure the image width and height are 320px
      img.style.width = IMAGE_SIZE + "px";
      img.style.height = IMAGE_SIZE + "px"; // Explicitly set height to 320px as well
