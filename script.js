...
function activateGroup(project) {
  ...
  // Set the target position for zoom out to the center of the canvas
  const centerX = window.innerWidth / 2;
  const centerY = window.innerHeight / 2;

  // Zoom-out target to the center of the canvas
  targetOriginX = centerX;
  targetOriginY = centerY;

  // Immediately show the description after the group is activated
  descriptions.classList.remove("visible"); // Hide description initially

  // Step 1: Zoom out to the center (without forming images yet)
  groupCameraLoop(); // Start the zoom-out animation

  // Step 2: Form the images after zoom-out
  setTimeout(() => {
    ...
    // Step 2: Form the group images immediately after zoom-out
    ...
    // Step 3: Now zoom in to the group after the images have been formed
    targetScale = 1;
    targetOriginX = window.innerWidth * 0.28 - CENTER_X;
    targetOriginY = window.innerHeight * 0.5 - CENTER_Y;

    // Step 3: Zoom in to the images and their description
    groupCameraLoop(); // Start the camera animation (zoom-in)

    // Unlock the camera after the zoom-in transition is finished
    setTimeout(() => {
      descriptions.classList.add("visible"); // Show the description box
      Array.from(descriptions.children).forEach(desc => {
        // Hide all descriptions
        desc.style.display = "none";
        // Only show the description that corresponds to the active project
        if (desc.dataset.project === project) {
      ...
      }, zoomDuration); // Use the same zoom duration as before

    }, 800); // Delay description appearance after zoom-in
